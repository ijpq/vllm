...
__forceinline__ __device__ void _collect_hist(
    typename cooperative_groups::grid_group& handle,
    int32_t* __restrict__ hist, int32_t* __restrict__ hist_ptr,
    int32_t* __restrict__ global_hist) {
    auto tid = threadIdx.x;
    if (tid < NUM_EXPERTS) atomicAdd(&hist_ptr[tid], hist[tid]);
    if (blockIdx.x == 0 && tid < NUM_EXPERTS)
        atomicAdd(&global_hist[tid], hist[tid]);
}
...
template<int NUM_EXPERTS>
__forceinline__ __device__ void _prefix_hist_CTA(typename cooperative_groups::grid_group& handle, int32_t* __restrict__ hist, int32_t*
__restrict__ prefix) {
    // prefix[i, :] = local_hist[i-1, :] + prefix[i-1, :] where i is CTA's ID
    int tid = threadIdx.x;
    int CTA_ID = blockIdx.x;
    if (CTA_ID > 0)
        if (tid < NUM_EXPERTS)
            prefix[CTA_ID * NUM_EXPERTS + tid] =  prefix[(CTA_ID -1 ) * NUM_EXPERTS + tid] + hist[tid];

    handle.sync();
    if (CTA_ID == 0) {
        for ( int bdx = 1; bdx < handle.num_blocks() ; bdx++) {
            if (tid < NUM_EXPERTS)
            prefix[bdx * NUM_EXPERTS + tid] += prefix[(bdx-1)*NUM_EXPERTS + tid];
            __syncthreads();
        }
    }
}
...
__global__ void fused_routing_kernel(
    ...
    int32_t* __restrict__ expt_offs_ptr, int32_t* __restrict__ hist_ptr, int32_t* __restrict__ prior_contrib, int32_t* __restrict__ hist_prefix,
...
//     int32_t* __restrict__ gate_index, int32_t* __restrict__ token_offs_pad_ptr,
//     int32_t* __restrict__ block_pid_map_ptr,
//     int32_t* __restrict__ expt_offs_ptr, int32_t* __restrict__ hist_ptr, int32_t* __restrict__ _prior_contrib_, int32_t* __restrict__ _hist_prefix,
//     int* padding_indices, int* padding_weights) {
...
#pragma unroll
    ...
    int32_t* prior_contrib_sm =
        reinterpret_cast<int32_t*>(sm_hist + expert_across_offset);
    if (local_tid < NUM_EXPERTS)
        prior_contrib_sm[local_tid] = prior_contrib[local_tid];
    ...
    */
    ...
    grid.sync();
    if (local_tid < NUM_EXPERTS)
        hist_sum[local_tid] = hist_prefix[local_tid];
    ...
    */
    ...
    switch (num_experts) {
        case 32: {
            ...
            size_t ROWS_PER_CTA =
                (num_tokens + numSMs * numBlocksPerSM - 1) / (numSMs*numBlocksPerSM);
...
#endif
            auto grid_dim = dim3(numSMs*numBlocksPerSM, 1, 1);
            // recompute config
            // if (cluster_size > hypo_cluster_size) {
            ...
            // TODO: we need to change this to grid group, and allocate an extra space for prior_contrib
            int32_t* prior_contrib;
            int32_t* hist_prefix;
            cudaMalloc(&prior_contrib, gridDim.x * num_experts);
            cudaMalloc(&hist_prefix, num_experts+1);
            void* args[] = { (void*)router_logits_ptr,
                               topk_weights_ptr, topk_indices_ptr, &max_n_tiles,
                               (void*)&num_tokens, gate_scale_ptr, topk_index_ptr,
                               gate_index_ptr, token_offs_pad_ptr,
                               block_pid_map_ptr, expt_offs_ptr, hist_ptr, prior_contrib, hist_prefix,
                               &padding_indices, &padding_weights};
            cudaLaunchCooperativeKernel((void*)kernel_wrapper, grid_dim, block_dim,args, compute_sm(), current_stream);
            break;
        }
//         case 128: {
//             auto warp_size = 32;
